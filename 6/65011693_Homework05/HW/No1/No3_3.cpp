...
class Book{
    ...
    public:
        ...
        //get a list of books that match specific genre from a collection of books
        // Return a vector of books that match the genre
        vector<Book> find_books_byGenre(vector<Book> books, Genre genre)
        {
            vector<Book> result;
            for (int i = 0; i < books.size(); i++)
            {
        ...
        // Sort a book by the title from a collection of books
        // Return a vector of books that sorted by the title
        vector<Book> sort_books_byTitle(vector<Book> books)
        {
            vector<Book> result;
            for (int i = 0; i < books.size(); i++)
            {
        ...
        // create a string for ISBN in the format of "123-456-789-A" 
        string create_ISBN_string()
        {
            string result = to_string(i.num1) + "-" + to_string(i.num2) + "-" + to_string(i.num3) + "-" + i.x;
            return result;
        }

        // Sort a book by the ISBN from a collection of books
        // Return a vector of books that sorted by the ISBN
        vector<Book> sort_books_byISBN(vector<Book> books)
        {
            vector<Book> result;
            for (int i = 0; i < books.size(); i++)
            {
                for (int j = i + 1; j < books.size(); j++)
                {
                    if (books[i].create_ISBN_string() > books[j].create_ISBN_string())
                    {
                        Book temp = books[i];
                        books[i] = books[j];
                        books[j] = temp;
...
int main()
{
    ...
    Book b2("Rust", "Peter", 2010);
    b2.set_ISBN(22, 111, 444, 'V');

    b1.set_enum(Genre::fiction);
    b2.set_enum(Genre::children);
    // cout << b1.get_genre() << endl;
    // cout<< b2.get_genre() << endl;

    // b2.compare_ISBN(b1);

    
    Book b3("zello World", "John", 2050);
    b3.set_ISBN(999, 999, 33, 'X');
    b3.set_enum(Genre::fiction);
    ...
    // test to sort a book by the title from a collection of books
    cout << "Test to sort a book by the title from a collection of books" << endl;
    vector<Book> result2 = b1.sort_books_byTitle(books);
    ...
    // test to sort a book by the ISBN from a collection of books
    cout << "Test to sort a book by the ISBN from a collection of books: " << endl;
    vector<Book> result3 = b1.sort_books_byISBN(books);
