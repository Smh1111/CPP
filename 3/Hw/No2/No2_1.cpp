...
int main() {
    ...
    sort(players.begin(), players.end(), comparePlayers_byMaxScores);
    ...
    for (const auto& player : players) 
    {
        try 
        {
            int maxScore = *max_element(player.scores.begin(), player.scores.end()); 
            cout << rank << ". " << left << setw(maxNameLength + 1) << player.name << right << setprecision(2) << maxScore ;
        }
